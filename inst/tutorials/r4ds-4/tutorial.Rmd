...
Dates-Times Key Functions: ymd_hms(), year(), month(), wday()
...
is.na(), summarise_all(), sum() pivot_longer
...
Data exploration: open_dataset(), collect(), glimpse()
...
Data inspection: select(), slice_head()
...
String pattern extraction: str_extract(), regex patterns, as.numeric()
...
Missing value analysis: where(), across(), str_detect(), summarise()
...
Date-time processing: ymd(), today(), date arithmetic, logical operations
...
Financial data inspection: column selection for analysis
...
Complex string parsing: case_when(), str_detect(), str_remove_all()
...
Advanced column selection: where() with custom functions
...
Pipeline start: Data loading and basic cleaning
...
Text cleaning: across(), where(), str_replace_all(), str_trim()
...
Date processing and missing values: ymd(), today(), drop_na()
...
Financial data parsing: Complex case_when() with multiple conditions
...
Complete pipeline: Data cleaning, factor analysis, advanced visualization with ggplotly()
