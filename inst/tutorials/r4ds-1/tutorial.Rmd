...
<!-- LL: do data transfomration before doing data visualization? Still need to create a plot for data transformation though, so maybe not? Or, hold off on the plot until the end of the section? -->
...
Since we have already loaded **tidyverse**, we have therefore loaded **tidyr** and by extension, **Billboard**. Type `billboard` into the Console and observe the rows and columns.
...
<!-- LL: should answer boxes be "billboards-1-test" or just "billboards-1"? --> 

###

### Exercise 2

Ask AI to create a simple graph plotting `wk1` against `wk2`. Provide a tibble of `billboard` for AI to reference. Specify to the AI to only use the functions `ggplot()`, `aes()`, and `geom_point()`. Copy and paste this code into a new code chunk with your QMD. Place your cursor on the line of code and run `Cmd/Ctrl + Enter`.
...
Ask AI to continue the code with `+` and add a trendline with `geom_smooth()`. Copy and paste this code to continue the previous code. Place your cursor on the line of code and run `Cmd/Ctrl + Enter`.
...
Ask AI to continue the code by adding a title, subtitle, caption, and axis labels using the `labs()` function. Don't worry if it isn't perfect. Copy and paste this code to continue the previous code. Place your cursor on the line of code and run `Cmd/Ctrl + Enter`.
