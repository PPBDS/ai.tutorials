...
Ask AI: Edit the code so that the `read_csv()` also to not read the first row as headings.
...
Ask AI: Edit the code so that the `skip` and `col_names` arguments are removed, and that the `read_csv()` is assigned to an object `df` and `read_csv()` is told with `col_types` that the column `artist.familiarity` is a double and to check if it fails. Then run `problems(df)`.
...
Ask AI: Based off of the `music.csv` tibble previously provided, create a similar tibble with only 3 rows of data using `tibble()`, and only with the first 3 columns from `music.csv`. Don't assign the tibble to an object.
...
Ask AI: Continue the previous code with the new tibble so that it is fed through a pipe and is written into a new csv file titled `music_fake.csv`.
...
read_csv(c("data/music_1.csv", "data/music_fake.csv"), id = "source")
```
