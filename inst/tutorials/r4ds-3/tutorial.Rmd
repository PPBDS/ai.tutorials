...
We begin by downloading the `babynames` package. Type in `install.packages(babynames)` into your console and press enter. 

Now, put `library(babynames)` in the same code chunk and place your cursor at the beginning of the line and use `Cmd/Ctrl + Enter` to execute that line.
...
`babynames` is currently a R package which is ready to use, but not as a database. To make this package into a database, we create a database connection using `DBI::dbConnect()`. The first argument selects the DBMS, then the second and subsequent arguments describe how to connect to it (i.e. where it lives and the credentials that you need to access it).
...
Ask AI how to connect the `babynames` package to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
```{r baby-names-2-test, echo = TRUE, eval = FALSE}
con <- DBI::dbConnect(
    duckdb::duckdb(),
    dbdir = "babynames"
  )
...
Restart your console. Since this is a new database, we need to start by adding some data. Ask AI how you can write the `babynames` data frame to a database table using `dbWriteTable()` with the variable `con`. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
```{r baby-names-3-test, echo = TRUE, eval = FALSE}

dbWriteTable(con, "babynames", babynames, overwrite = TRUE)
...
Ask AI how to start a pipe from `con` to retrieve the contents of the `babynames` dataset using `dbReadTable()` and read it using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
You can use `dbGetQuery()` to get the results of running a query on the database. Ask AI how to get the results of the query by using `dbGetQuery()`. Make sure to provide your code of the DBI connection and the SQL query.  Add this code to the same code chunk in your QMD and run `Cmd/Ctrl + Enter`.
...
We begin by downloading the `nycflights13` package. Type in `install.packages(nycflights13)` into your console and press enter. 

Now, put `library(nycflights13)` in the same code chunk and place your cursor at the beginning of the line and use `Cmd/Ctrl + Enter` to execute that line.
...
Ask AI how to connect the `flights` table from `nycflights13` to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
Restart your console. Since this is a new database, we need to start by adding some data. Ask AI how you can write the `flights` table from the `nycflights13` package to a database using `dbWriteTable()` with the variable `con`. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
You can check that the data is loaded correctly by using the `dbListTables()` function on `con` to lists all tables in the database.


### Exercise 4

Ask AI how to start a pipe from `con` to retrieve the contents of the `flights` dataset using `dbReadTable()` and read it using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
Ask AI how to check the column type of tailnum from the flights dataset using `class()`. Add the code to your Console and hit `Enter`.
