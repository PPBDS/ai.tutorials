...
# Write the babynames dataset to the database
DBI::dbWriteTable(con, "babynames", babynames::babynames, overwrite = TRUE)

# Read the babynames table as a tibble
con |> 
  dbReadTable("babynames") |> 
  as_tibble()

# Create a dbplyr connection to the babynames table
babynames_db <- tbl(con, "babynames")

# Example query: Filter names starting with "A" after 2000 with at least 10 occurrences
x <- babynames_db |>
...
# Example query: Top 5 longest names starting with "A" in 2009
...
Place your cursor in the QMD file at the starting of the `library(tidyverse)` line. Use `Cmd/Ctrl + Enter` to execute that line.
...
Ask AI how to connect the `babynames.duckdb` database as a table using `DBI::dbConnect()` by assigning it to a variable `con`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
Restart your console. Ask AI how you can retrieve the contents of the `babynames.duckdb` dataset using `dbReadTable()` and read it using `as_tibble()`. Make sure the result is assigned to a variable. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
`dbReadTable()` has the argument of `babynames` instead of `babynames.duckdb` because `babynames.duckdb` the database file name, not the name of the table inside the database.`dbReadTable()`returns a data frame, not a tibble, so we pipe it into `as_tibble()` for the result to be a tibble.
...
MAKE EXERCISE HERE TO READ THE SQL PROMPT
as_tibble(dbGetQuery(con, sql))

### Exercise 6

 You just used SQL to filter the babynames database directly. Although, instead of writing raw SQL yourself, we can do the exact same task with R code using `dbplyr`. `dbplyr` is a `dplyr` backend, which means that you keep writing `dplyr` code but the backend executes it differently. In this, `dbplyr` translates to SQL; other backends include `dbplyr` which translates to `data.table`, and `multidplyr` which executes your code on multiple cores. 
...
Ask AI how to use `dbplyr` to filter the data to show all names start with A after the year 2000 using `str_starts()`. Paste this code into the same code chunk and place your cursor on a line of the code and run `Cmd/Ctrl + Enter`.
...
Prompt AI to continue the pipe by using `.by()` to summarize the data by grouping by `year` and `length_class`, calculating the total number of babies in each group by summing `n` using `sum()`. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. Next, prompt AI to continue the pipe to summarize the data by grouping by year and length_class, calculating the total number of babies in each group by summing n. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
