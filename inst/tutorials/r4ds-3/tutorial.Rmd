...
#babynames plot
x <-  dbplyr_names |>
filter(substr(name, "A"), year > 2000, n >= 10) |>
...
Ask AI how to use `dbplyr` to filter the data to show all names start with A after the year 2000 using `substr()`. Paste this code into the same code chunk and place your cursor at the start of `dbplyr_names` and run `Cmd/Ctrl + Enter`.
...
top_a_2009 <-  dbplyr_names |>
  filter(year == 2009, substr(name, "A")) |>
...
Ask AI how to connect the `flights` table from `nycflights13` to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_air`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
con_air <- DBI::dbConnect(
    duckdb::duckdb(),
    dbdir = "nycflights13.duckdb"
  )
```

### 

You can check that the data is loaded correctly by using the `dbListTables()` function on `con_air` to lists all tables in the database.
