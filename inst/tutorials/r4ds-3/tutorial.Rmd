...
You will learn about how to work with numbers using commands such as [`pmin()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Extremes.html) with the **nycflights13** package.

You will learn about some of the power of the [**stringr**](https://stringr.tidyverse.org/) package: how to create, combine, and extract strings with the [**babynames**](https://hadley.github.io/babynames/) package.

### Exercise 1

Create a Github repo called `transformation`. Make sure to click the "Add a README file" check box.
...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"Transforming Data"` -- and an author (you). Render the document and save it as `analysis.qmd`.
...
In your QMD, put `library(tidyverse)` in a new code chunk. Render the file with `Cmd/Ctrl + Shift + K`.
...
<!-- XX: Insert a knowledge drop related to this project. -->
<!-- SG: Missing cache:true exercise and type x in console exercise-->
### Exercise 11
...
<!-- SG: Why are we working on the same QMD? -->
<!-- SG: Should we change this so we start by making a new repo and QMD? -->
### Exercise 1

In the setup chunk of the QMD, load `babynames` with the `library()` function. 
...
We load **babynames** into our current QMD so it is accessible to use with the rest of the code in the document. The `babynames` is a data set which contained names used for babies from 1880 to 2017. If you want to explore the data set, checkout [popular baby names](https://www.ssa.gov/oact/babynames/limits.html).
...
Typing out `babynames` is the same as typing out `print(babynames)`. You might be wondering what tibble is, A tibble is a new form of a data frame in R that is part of the tidyverse library, and tibbles print the data in a more efficient format than a data frame, showing the values of the columns, their datatype, and the size of the dataset.
<!-- SG: Is it necessary to have the same knowledge drop twice? -->

### Exercise 3

Using your favorite AI, prompt it to generate R code that takes `babynames` and filters the dataset to show all names that have a first name starting with A. Make sure to provide a tibble of **babynames** for the AI to use. If necessary, change the code so that the result of the pipe is not assigned to a variable. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
The function `str_starts()` takes a column (in this case it is `name`), and selects the rows where the string of name starts with A. To check for `NA` values, use `any()` function with argument `is.na()`, argument of which should be the dataset babynames. 
...
Strings can be created using either single quotes (') or double quotes ("). If you wanted to include single quotes in a string you could put the outer quotes as double quotes.
...
Insert a knowledge drop related to this project.
...
Go to the AI and ask it to generate R code that plots a basic graph showing the proportion of names starting with A over the years for each sex. Mention you want to use the data from `x` and copy and paste the `x` you ran in the Console with the resulting tibble. You only need the top 3 lines, mainly to include column names.
...
If you were to not use `str_c()`, double quotes "" or single quotes '' will also create a string. To include a literal single or double quote, you can use the backslash `\` to escape it. For example, `"\""` will return `'"'`. To include a literal backlash in your string, it is pretty simple and all you need to do is have two backslashes in a string to have one literal one. 


### Exercise 11

Once again, place your cursor on the line with the new `str_c()` code and hit `Cmd/Ctrl + Enter`, but don't run it yet. Now edit the code so that within `str_c()`, the strings include a backslash `\` at the start and doublequotes `""` at the end. Change the assigned variable to `title2` and press `Enter` to run the code. 

Then, type `str_view(title2)` in the Console, and press `Enter` to view the string.
...
```{r, echo=TRUE}
title2 <- str_c("\\Proportion of Babies with Names Starting with ", "'A'", " Over Time\"\"")
str_view(title2)
```
...
```{r, echo=TRUE}
subtitle1 <- str_glue("Data from {min(x$year)} to {max(x$year)} shows that the proportion of names starting with 'A' was lowest around the 1950s")
subtitle1
```
...
Copy and paste the new `str_glue()` code into the Console, but don't run it yet. Now manually edit the code so that at the start, there is `\t`, and there is `\n` before the final word. Change the assigned variable to `subtitle2` and press `Enter` to run the code.

Then, type `str_view(subtitle2)` in the Console, and press `Enter` to view the string.
...
```{r, echo=TRUE}
subtitle2 <- str_glue("\tData from {min(x$year)} to {max(x$year)} shows that the proportion of names starting with 'A' was lowest around the \n1950s")
str_view(subtitle2)
```
...
Using the same AI, ask it to generate code for finding the top five most common names beginning with 'A' in 1950. 

Here is the prompt we used: using R, create code that finds the top five most common names beginning with 'A' in 1950. First use `filter()` and `slice_head()` to get the top names of the 'A' name column ordered by highest proportion, then use `pull()` to create a vector of the column, and finally use `str_flatten()` to create a string. Use `str_c()` to print out a coherent sentence at the end. 
...
<!-- SG: This knowledge drop seems too long. -->
