...
con_babynames <- dbConnect(duckdb::duckdb(), 
				 dbdir = "data/babynames.duckdb")
con_flights <- dbConnect(duckdb::duckdb(), 
				 dbdir = "data/flights.duckdb")

# Create a dbplyr connection to the babynames table

babynames_db <- tbl(con_babynames, "babynames")
...
#Flights
flights_db <- tbl(con_flights, "flights")
...
Ask AI how to connect the `babynames` package to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_babynames`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
as_tibble(dbGetQuery(con_babynames, sql))
```
...
Ask AI how to start a pipe from `con` to retrieve the contents of the `flights` dataset using `dbReadTable()` and convert it to a tibble using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
