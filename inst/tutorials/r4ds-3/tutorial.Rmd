...
#Flights
 dbplyr_nycflights <- tbl(con_air, "flights")
...
Ask AI how to connect the `babynames` package to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_names`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
### 
Our code:
```{r baby-names-13-test, echo = TRUE, fig.keep = FALSE}
...
View the data by putting  dbplyr_nycflights in your console. Copy/paste the resulting dataframe.
...
Ask AI to create a new tailnum column using parse so we can extract numbers from a character column. Also, ask AI to clean flights and select the `dep_time`, `sched_dep_time`, `dep_delay`, and `tailnum` columns. Add the code to your QMD in a new chunk. Assign this pipe to `departure_data`. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
