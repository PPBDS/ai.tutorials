...
We begin by downloading a DuckDB version of the babynames dataset directly from GitHub using `download.file()`. This adds the database file, `nameby_year.duckdb`, to our working directory so we can use it in this tutorial.
...
Ask AI how to connect the `nameby_year.duckdb` database to `duckdb` using `DBI::dbConnect()` by assigning it to a variable `con_names`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
Ask AI how to start a pipe from `con` to retrieve the contents of the `babynames` table from the `nameby_year.duckdb` database using `dbReadTable()` and read it using `as_tibble()`. Make sure the result is not assigned to a variable. Add this code in a new code chunk to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. 
...
We begin by downloading a DuckDB version of the flights dataset directly from GitHub using download.file(). This stores the database file (nameby_year.duckdb) in our working directory so we can use it in this tutorial. In the Console, run:
