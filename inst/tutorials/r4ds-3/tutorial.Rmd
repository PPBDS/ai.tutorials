...
This section covers [Chapter 14: Strings](https://r4ds.hadley.nz/strings.html), [Chapter 12: Logical Vectors](https://r4ds.hadley.nz/logicals.html), and [Chapter 21: Databases](https://r4ds.hadley.nz/databases.html) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. You will learn about some of the power of the [**dbplyr**](https://dbplyr.tidyverse.org/) package and how it helps to connect and work with duck databases and SQL. You will also learn about some of the[**stringr**](https://stringr.tidyverse.org/) package: how to create, combine, and extract strings, and logicial operations with `if_else()`. Some important functions which we will learn include: [`str_c()`](https://stringr.tidyverse.org/reference/str_c.html), [`str_flatten()`](https://stringr.tidyverse.org/reference/str_flatten.html), [`dbWriteTable()`](https://rdrr.io/cran/DBI/man/dbWriteTable.html), [`dbConnect()`](https://rdrr.io/cran/DBI/man/dbConnect.html)and more.
...
We will be working with databases in this tutorial. There are three high level differences between data frames and database tables:
...
We begin by downloading a DuckDB version of the babynames dataset directly from GitHub using `download.file()`. This adds the database file, `babynames.duckdb`, to our working directory so we can use it in this tutorial.
...
```{r babynames-1}
=======
>>>>>>> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
question_text(NULL,
	answer(NULL, correct = TRUE),
	allow_retry = TRUE,
	try_again_button = "Edit Answer",
...
`babynames.duckdb` is currently a collection of binary files and is not ready to use until we create a database connection using `DBI::dbConnect()`. The first argument selects the DBMS, then the second and subsequent arguments describe how to connect to it (i.e. where it lives and the credentials that you need to access it).

<!-- DK: Knowledge drop? Maybe: "Note that `babynames.duckdb` is not a ready to use database. It is a binary file with all the information we need in order to create a database." Or something?  -->

# \<\<\<\<\<\<\< HEAD

### Exercise 2

DK: DANGER! DANGER! \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22

### Exercise 2

Setting up a client-server or cloud DBMS would be a pain for this tutorial, so we’ll instead use an in-process DBMS that lives entirely in an R package: `duckdb`. Thanks to the magic of DBI, the only difference between using `duckdb` and any other DBMS is how you’ll connect to the database.

Ask AI how to connect the database as a table using `DBI::dbConnect()`, so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
<!-- DK: I should not have to execute this command: DBI::dbWriteTable(con, "babynames", babynames::babynames, overwrite = TRUE) -->

Our code:

```{r baby-names-2-test, echo = TRUE}
con <- DBI::dbConnect(
    duckdb::duckdb(),
...
\<\<\<\<\<\<\< HEAD Ask AI how you can retrieves the contents of the babynames.duckdb dataset using `dbReadTable()`. Add this code to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. =======

### Exercise 3

Ask AI how you can view the babynames.duckdb dataset using dbReadTable(). Add this code to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
Using your favorite AI, prompt it to generate SQL code that takes `babynames` and filters the dataset to show all names that have a first name starting with A after the year 2000. Make sure to provide some lines of **babynames** for the AI to use. If necessary, change the code so that the result of the pipe is not assigned to a variable. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.
...
\<\<\<\<\<\<\< HEAD You just used SQL to filter the babynames database directly. Although, instead of writing raw SQL yourself, we can do the exact same task with R code using `dbplyr`. `dbplyr` is a `dplyr` backend, which means that you keep writing `dplyr` code but the backend executes it differently. In this, `dbplyr` translates to SQL; other backends include `dbplyr` which translates to `data.table`, and `multidplyr` which executes your code on multiple cores. ======= You just used SQL to filter the babynames database directly. Although, instead of writing raw SQL yourself, we can do the exact same task using dbplyr. dbplyr is a dplyr backend, which means that you keep writing dplyr code but the backend executes it differently. In this, dbplyr translates to SQL; other backends include dbplyr which translates to data.table, and multidplyr which executes your code on multiple cores. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
To use `dbplyr`, you must first use `tbl()` to create an object that represents a database table. Ask AI how to use `dbplyr` to filter the data to show all names start with A after the year 2000. Paste this code into the same code chunk and place your cursor on a line of the code and run `Cmd/Ctrl + Enter`.
...
```{r baby-names-8a-test, echo = TRUE}
babynames_db <- tbl(con, "babynames")
babynames_db |>
...
\<\<\<\<\<\<\< HEAD Prompt AI to give you code that creates a new column of names classified as "Short" or "Long". Anything less than or equal to 5 can be classifed as short. Add this code as a continuation of your pipe to the same chunk in your QMD. Since the `babynames` is a data set which contains names used for babies from 1880 to 2017 in America, we are using the average length of an American name (5.44) truncated.

======= Next, prompt AI to give you code that creates a new column of names classified as "Short" or "Long". Anything less than or equal to 5 can be classifed as short. Add this code as a continuation of your pipe to the same chunk in your QMD. Since the `babynames` is a data set which contains names used for babies from 1880 to 2017 in America, we are using the average length of an American name (5.44) truncated. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22 In the Console, run:
...
\<\<\<\<\<\<\< HEAD Prompt AI to continue the pipe to summarize the data by grouping by `year` and `length_class`, calculating the total number of babies in each group by summing `n`. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. ======= Next, prompt AI to continue the pipe to summarize the data by grouping by year and length_class, calculating the total number of babies in each group by summing n. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
\<\<\<\<\<\<\< HEAD Prompt the AI to arrange the summarized results in ascending order by `year`, and collect the results from the database into a local R tibble for further use, such as plotting. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. ======= Now, prompt the AI to arrange the summarized results in ascending order by year, and collect the results from the database into a local R tibble for further use, such as plotting. Add the code to your QMD in the same chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
```{r baby-names-15-test, echo = TRUE}
x <- babynames_db |>
  filter(str_starts(name, "A"), year > 2000, n >= 10) |>
  mutate(length_class = if_else(str_length(name) <= 5, "Short", "Long")) |>
  summarize(total = sum(n), .by = c(year, length_class)) |>
...
Here is the prompt we used: Using R with a database connection `babynames_db`, create code that finds the top five longest names beginning with 'A' in 2009. First use `filter()` to get names starting with 'A' in 2009, then create a `name_length` column using `mutate()` and `str_length()`. Since you're working with a database backend, use `collect()` to bring the data into R memory, then use `arrange()` with `desc()` to order by name length and `slice_head()` to get the top 5. Finally, use `pull()` to extract the name column as a vector and use `str_flatten()` to create a string. Use `str_c()` to print out a coherent sentence at the end.
...
```{r baby-names-16-test, echo = TRUE}
top_a_2009 <- babynames_db |>
...
insight_text <- top_a_2009 |>
  pull(name) |>
  str_flatten(collapse = ", ", last = " and ")

insight <- str_c(
  "In 2009, the longest 'A' names were ", 
  insight_text, 
  "."
)
...
The `desc()` function is used to create a descending order. This means that rows with longest names will appear first in the data frame. \###
...
This section covers key concepts from [Chapter 13: Numbers](https://r4ds.hadley.nz/numbers.html) and [Chapter 15: Regular Expressions](https://r4ds.hadley.nz/regexps.html) from [*R for Data Science (2e)*](https://r4ds.hadley.nz/) by Hadley Wickham, Mine Çetinkaya-Rundel, and Garrett Grolemund. You will learn about some of the power of working with numeric data and text patterns using core Tidyverse packages: readr, dplyr, and stringr. Important functions you'll learn about include: [parse_number()](https://readr.tidyverse.org/reference/parse_number.html) [is.na()](https://rdrr.io/r/base/NA.html) [str_detect()](https://stringr.tidyverse.org/reference/str_detect.html) [pmax()](https://rdrr.io/r/base/Extremes.html) We will also learn about integer division and moduluo.
...
We begin by downloading a DuckDB version of the flights dataset directly from GitHub using download.file(). This stores the database file (babynames.duckdb) in our working directory so we can use it in this tutorial. In the Console, run:

```         
download.file(
  "https://github.com/PPBDS/ai.tutorials/raw/refs/heads/main/inst/extdata/flights.duckdb",
...
Ask AI how to connect and write the database as a table so it is accessible to use with the rest of the code in the document. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. In the Console, run:
...
You can check that the data is loaded correctly by using the `dbListTables()` function that lists all tables in the database. Try running `dbListTables()` on `con`.

### Exercise 3

Ask AI how you can view the flights.duckdb dataset using dbReadTable. Add this code to the QMD. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. Copy and paste the resulting tibble.
...
`dbGetQuery()` is a function which takes a connection and an SQL query as a text string as inputs. It returns the result of that query as a data frame. <!-- SG: How do I make this sound less random? -->
...
View the data by putting flights_db in your console. Copy/paste the resulting dataframe.
...
\<\<\<\<\<\<\< HEAD View the tailnum column by pasting this in your console: ======= Now, view the tailnum column by pasting this in your console: \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22

```         
flights$tailnum
```

Ask AI how to check the column type of tailnum from the flights dataset. Add the code to your Console and hit `Enter`.
...
Ask AI to create a new tailnum column using parse so we can extract numbers from a character column. Also, ask AI to clean flights and select the dep_time, sched_dep_time, dep_delay, and tailnum columns. Add the code to your QMD in a new chunk. Assign this pipe to "flights_clean". Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`.

x \<- c(1, 2, 3, 4, NA) ranktypes \<- tibble(x = x)
...
\<\<\<\<\<\<\< HEAD CP/CR. ======= CP/CR \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
\<\<\<\<\<\<\< HEAD Ask your favorite AI to write code that summarizes how many missing values are in the `dep_time` and `arr_time` columns using the function `is.na()`. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. ======= Now, ask your favorite AI to write code that summarizes how many missing values are in the `dep_time` and `arr_time` columns using the function `is.na()`. Add the code to your QMD in a new chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
...
A tibble: 1 × 2 missing_dep missing_arr <int> <int> 1 8255 8713
...
Ask AI how you can continue your previous pipe and filters out rows where dep_time is missing using !is.na(). Add the code to your QMD in the same chunk as an addition to the pipe. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. View your new cleaned dataset.
...
\<\<\<\<\<\<\< HEAD Ask AI how you can find tail numbers starting with "N1". Add the code to your QMD in a new code chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. ======= Now, ask AI how you can find tail numbers starting with "N1". Add the code to your QMD in a new code chunk. Place your cursor on the first line of the code and run `Cmd/Ctrl + Enter`. \>\>\>\>\>\>\> 7b55ab187a84ce0c0bf498b5335d72e18dde9f22
