...
Place your cursor in the QMD file on the `library(tidyverse)` line. Use `Cmd/Ctrl + Enter` to execute that line. Do this again for the `library(readxl)` line and the `library(dbplyr)` line. 
<!-- SG: Should this be one exercise or worded differently? -->
...
Add this code as a replacement to your current code in the same code cell in your QMD. Run `Cmd/Ctrl + Enter`.
...
Add the resulting code as a replacement of your current code to the code chunk in your QMD. Run `Cmd/Ctrl + Enter`.
...
Ask AI how to pipe your filtered data into a dot plot using `geom_point()` where you map `country` to the x-axis, `milk` to the y-axis, and `n` to the size of the points. Also map `country` to `color` for better distinction between countries. Add this code as a continuation of your current pipe in the same code chunk in your current QMD. Place your cursor at the beginning of the line and run `Cmd/Ctrl + Enter`.
...
Fix this by asking AI to use our pipe from the last exercise and add `geom_text()` to display the count values on top of each bar and use `position_dodge()` in `geom_text()` to align the text with the bars. Within `theme()` change the x-axis text to be angled at 45 degrees using `element_text`.
 ...
 Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `read_excel("data/cheeses.xlsx") |> ` and run `Cmd/Ctrl + Enter`.
...
Add this code as a replacement to your current code chunk in the same QMD. Place your cursor at the beginning of the line where it says `read_ excel` and run `Cmd/Ctrl + Enter`.
...
Place your cursor at the beginning of the line where it says `read_excel("data/cheeses.xlsx") |>` and run `Cmd/Ctrl + Enter`.
...
Place your cursor on the line where the pipe is assigned to `cheese_tibble`, run `Cmd/Ctrl + Enter`. Now, the workspace also includes a copy of `cheese_tibble`.
...
<!-- DK: Default is `Cmd/Ctrl + Enter` followed by CP/CR. --> done.
...
Ask AI to  continue the pipe into the function `geom_jitter()` . Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `read_excel("data/wine.xlsx")` and run run `Cmd/Ctrl + Enter`.
...
Ask the same AI to set the `method` argument to `loess` for `geom_smooth()`. Add this code as a replacement of `method = "lm"` to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel("data/wine.xlsx")` and run `Cmd/Ctrl + Enter`.
...
Ask AI to add a `theme()` argument. Set this to `classic` Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `read_excel("data/wine.xlsx") ` and run `Cmd/Ctrl + Enter`.
...
Ask AI how to select the `Phenols`, `Flavanoids`, and `Color` columns from `wine` using `select()`. Add this code as a replacement to your current code to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel` and run `Cmd/Ctrl + Enter`.
...
Place your cursor on the line where the pipe is assigned to `wine_tibble`, run `Cmd/Ctrl + Enter`. Now, the workspace also includes a copy of `wine_tibble`.

<!-- DK: Nit wine_tibble but wine_tibble or wine_data or just wine. -->
...
### Exercise 16 
<!-- SG: I only have 16 exercises in both sections. Is this too little? Not sure what other exercises to add though. -->
