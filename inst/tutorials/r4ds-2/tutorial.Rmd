...
Ask AI how to read a `.xlsx` spreadsheet called `data/cheeses.xlsx` using `read_excel()`. Add this code to a new code chunk in your QMD. Make sure the code is not assigned to a variable. Place your cursor at the beginning of the line where it says `read_excel()` and run `Cmd/Ctrl + Enter`.
...
Add this code as a replacement to your current code in the same code cell in your QMD. Run `Cmd/Ctrl + Shift + K`.
...
Paste our prompt into your favorite AI to filter the top 3 countries in the data through a single pipe.
...
Add the resulting code as a replacement of your current code to the code chunk in your QMD. Run `Cmd/Ctrl + Shift + K` to render. 
...
Fix this by asking AI to use our pipe from the last exercise and add `geom_text()` to display the count values on top of each bar and use `position_dodge()` in `geom_text()` to align the text with the bars. Within `theme()` change the x-axis text to be angled at 45 degrees using `element_text`.
 ...
 Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `read_excel("data/cheeses.xlsx") |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI how to pipe your filtered data into a stacked bar chart using `geom_bar()` with the `position` argument set to `stacked` to explore the relationship between the `country` column and `milk` column. Set the `weight` to `n`. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `read_excel("data/cheeses.xlsx") |>` and run `Cmd/Ctrl + Shift + K` to render.
...
Paste our pipe into AI and ask it to change our code so that the plot is polar using `coord_polar()`. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `read_excel("data/cheeses.xlsx") |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Make another plot by asking AI how to pipe `xc` into a faceted column plot using `geom_col()` and `facet_wrap` that shows the number of cheeses by milk type using the columns `milk` and `n` and map the `fill` to `milk`. 

Mention you want to use the data from `xc` and copy/paste the `xc` you ran in the Console with the resulting tibble. You only need the top 3 lines, mainly to include column names.

Add this code in a new code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to pipe `read_excel("data/wine.xlsx")` into a bar plot with `aes` only mapped to `x = Type` using `geom_bar()`. Add this code to a new code chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask the same AI to set the `method` argument to `loess` for `geom_smooth()`. Add this code as a replacement of `method = "lm"` to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to use `geom_text()` within the pipe without creating a variable to find the highest value of `Flavanoids` using the `max()` and `filter()` functions and label this as `Outlier`. Set the `color` to `magenta` and `fontface` to bold. Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
