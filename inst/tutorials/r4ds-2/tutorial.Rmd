...
top_countries <- read_excel("data/cheeses.xlsx") |>
  filter(!is.na(country)) |>           
  count(country, sort = TRUE) |>
  slice_head(n = 3) |>
  pull(country)
xc <- read_excel("data/cheeses.xlsx") |>
...
<!-- DK: Add data directory question. -->

<!-- DK: Change download.file() to that directory. -->
...
<!-- DK: Add the links to functions and to the packages. Function names have backticks. -->

You will learn about working with categorical and continuous variables using different types of variation and exploratory data analysis using core packages such like **[readxl](https://readxl.tidyverse.org/)**, **dbplyr**, and **ggplot2**. Important functions include: facet_wrap(), coord_flip(), geom_jitter(), geom_smooth(), scales_  , count(), slice_head(), and mutate(). 
...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"Analyzing Cheese and Wine"` -- and an author (you). Render the document and save it as `analysis.qmd`.
...
We begin by downloading our spreadsheet `data/cheeses.xlsx` directly from GitHub using `download.file()`. 

This adds the spreadsheet, `data/cheeses.xlsx`, to our working directory so we can use it in this tutorial.
...
Ask AI how to read a `.xlsx` spreadsheet called `data/cheeses.xlsx` using `read_excel()`. Add this code to a new code chunk in your QMD. Place your cursor at the beginning of the line where it says `read_excel()` and run `Cmd/Ctrl + Enter`.
...
```{r cheese-2-test, echo = TRUE, eval = FALSE}
...
Ask AI to give you code to make a stacked bar plot using `geom_bar()` to explore the relationship between countries and the milks they use to produce different cheeses by using the `country` and `milk` columns. Mention you want to use the data from `data/cheeses.xlsx` and copy and paste the `read_excel("data/cheeses.xlsx")` you ran in the Console with the resulting dataframe. You only need the top 3 lines, mainly to include column names.

Add this code in a new code cell in your QMD. Run `Cmd/Ctrl + Shift + K`.
...
Our plot is crowded because it includes all countries and has many `NA`values. We need to identify the top 3 countries by filter our data accordingly.

Ask AI to help you create a variable called `top_countries` that uses `data/cheeses.xlsx` and pipes it into `filter()` with `!is.na()` to remove rows where country is `NA`, continues the pipe with `count()` to count occurrences by country and sort them, then uses `slice_head(n = 3)` to get the top 3 countries and finally uses `pull(country)` to extract just the country names as a vector.

Add this code as a replacement of your current code to the code chunk in your QMD. Run `Cmd/Ctrl + Shift + K` to render. Then type `top_countries` in the Console to see the results.
...
Ask AI to start with `read_excel("data/cheeses.xlsx")` and pipe it into `filter()` to keep only countries in `top_countries` while filtering out `NA` values in the `milk` column using `!is.na(milk)`. Then use `count(country, milk)` to count combinations of country and milk type, group by `country` with `group_by()`, and arrange the data in descending order by `n`.

Add this code to a new code chunk in your QMD. Run Cmd/Ctrl + Shift + K to render. 
...
Within the recent code chunk, add the option: `#| cache: true`. Assign the result of our pipe to `xc`. 
...
Place your cursor on the line where the pipe is assigned to `xc`, run `Cmd/Ctrl + Enter`. Now, the workspace also includes a copy of `xc`.
...
Within the Console, type `xc`, which we previously assigned to a pipe and ran in the Console. Hit `Enter`.
...
Ask AI how to pipe `xc` into a dot plot using `geom_point()` where you map `country` to the x-axis, `milk` to the y-axis, and `n` to the size of the points. Also map `country` to `color` for better distinction between countries. Add this code in a new code chunk in your QMD. Place your cursor at the beginning of the line and run `Cmd/Ctrl + Shift + K` to render.
...
Paste our pipe from the last question and ask AI to change the pipe so that it makes a grouped bar chart using `geom_col()` with `position = "dodge"` to separate bars by country. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Fix this by asking AI to use our pipe from the last exercise and add `geom_text()` to display the count values on top of each bar and use `position_dodge()` in `geom_text()` to align the text with the bars. Within `theme()` change the x-axis text to be angled at 45 degrees using `element_text`.
 ...
 Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI how to pipe `xc` into a stacked bar chart using `geom_bar()` to explore the relationship between the `country` column and `milk` column. Set the `weight` to `n`. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Paste our pipe into AI and ask it to change our code so that the plot is polar using `coord_polar()`. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to change the code so that `theta` is set to `y` within `coord_polar()`. This makes the plot look like a donut plot rather than a pie chart. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Make another plot by asking AI how to pipe `xc` into a faceted column plot using `geom_col()` that shows the number of cheeses by milk type using the columns `milk` and `n` and map the `fill` to `milk`. Add this code as a replacement of your current code in the same code chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI how to continue the pipe by adding `coord_flip()` and add use `show.legend = FALSE` so that there is no legend. Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI how to continue your current pipe and use `scale_y_sqrt()` to make the scaling better and add `theme_gray()`.Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to use the `labs()` function and add a `title`, `subtitle`, `caption`, and labels for the `x` and `y` axes. Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor at the beginning of the line where it says `xc |> ` and run `Cmd/Ctrl + Shift + K` to render.
...
We begin by downloading our spreadsheet `data/wine.xlsx` directly from GitHub using `download.file()`. 

This adds the spreadsheet, `data/wine.xlsx`, to our working directory so we can use it in this tutorial.
...
Ask AI how to read a `.xlsx` spreadsheet called `data/wine.xlsx` using `read_excel()`. Add this code to a new code chunk in your QMD. Place your cursor at the beginning of the line and run `Cmd/Ctrl + Enter`. Type `wine` in the console to retreive the contents of `wine`.
...
```{r wine-2-hint-1, echo = TRUE, eval = FALSE}
...
You can also create a brand new Google sheet with `gs4_create()` or write to an existing sheet with `sheet_write()`.

### Exercise 4

Ask AI to pipe `read_excel("data/wine.xlsx")` into a bar plot with `aes` only mapped to `x = Type` using `geom_bar()`. Add this code to a new code chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel` and run `Cmd/Ctrl + Enter`. 
...
On the x-axis, the chart displays `Type`, a variable from `wine`. On the y-axis, it displays `count`, but `count` is not a variable in `wine`. Where does count come from? Many graphs, like scatterplots, plot the raw values of your dataset. The algorithm used to calculate new values for a graph is called a stat, short for statistical transformation.

`?geom_bar` shows that the default value for `stat` is “count”, which means that `geom_bar()` uses `stat_count()`.

### Exercise 5

Ask AI how to select the `Phenols`, `Flavanoids`, and `Color` columns from `wine` using `select()`. Add this code as a replacement to the current pipe with a new pipe from `read_excel` to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel` and run `Cmd/Ctrl + Enter`. 
...
Within the recent code chunk, add the option: `#| cache: true`. Assign the result of our pipe to `xw`. 

`Cmd/Ctrl + Shift + K`. By including `#| cache: true` you cause Quarto to cache the results of the chunk. The next time you render your QMD, as long as you have not changed the code, Quarto will just load up the saved fitted object.

Place your cursor on the line where the pipe is assigned to `xw`, run `Cmd/Ctrl + Enter`. Now, the workspace also includes a copy of `xw`.
...
```{r wine-7-hint-1, echo=TRUE, eval=FALSE}
xw <- read_excel("data/wine.xlsx")|>
  select(Phenols, Color, Flavanoids)
```
...
Within the Console, type `xw`, which we previously assigned to a pipe and ran in the Console. Hit `Enter`.
...
Ask AI to start a pipe from `xw` to make a plot using `geom_point()` and the columns `Phenols` and `Flavanoids`. Set `color` argument to `Color` column. Add this code to a new code chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to  continue the pipe into the function `geom_jitter()` . Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask the same AI to set the `method` argument to `loess` for `geom_smooth()`. Add this code as a replacement of `method = "loess"` to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to use `geom_text()` within the pipe without creating a variable to find the highest value of `Flavanoids` using the `max()` and `filter()` functions and label this as `Outlier`. Set the `color` to `red` and `fontface` to bold. Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to add a `theme()` argument. Set this to `classic` Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
...
Ask AI to use `labs()` and edit or add a proper title, subtitle, and caption. If axis labels are appropriate, add them, but if unnecessary, don't bother. Don't assign the code for the plot to any variable. Add this code as a continuation of your pipe to the same chunk in your QMD.  Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Shift + K` to render.
