...
Ask AI how to read a `.xlsx` spreadsheet called `cheeses.xlsx` using `read_excel()` and assign the result to a variable `cheese`. Add this code to a new code chunk in your QMD. Place your cursor at the beginning of the line where it says `cheese <- ...` and run `Cmd/Ctrl + Enter`. Type `cheese` in the console to retrieve the contents of `cheese`.
...
Our plot is very crowded with all the countries forming a grey box and the legend takes up most of the plot. `cheese` also has a lot of `NA` values. 
...
Ask AI how to pipe `xc` into a stacked bar chart using `geom_bar()` to explore the relationship between the `country` column and `milk` column. Add this code in a new code chunk in the same QMD. Place your cursor at the beginning of the line where it says `x |>` and run `Cmd/Ctrl + Enter`. 
...
Make anothe plot by asking AI how to pipe `xc` into a faceted column plot using `geom_col()` that shows the number of cheeses by milk type using the columns `milk` and `n` and map the `fill` to `milk`. Add this code as a replacement to your current code in the same code chunk in the same QMD. Place your cursor at the beginning of the line where it says `xc |>` and run `Cmd/Ctrl + Enter`. 
...
Ask AI how to read a `.xlsx` spreadsheet called `data/wine.xlsx` using `read_excel()`. Add this code to a new code chunk in your QMD. Place your cursor at the beginning of the line and run `Cmd/Ctrl + Enter`. Type `wine` in the console to retreive the contents of `cheese`.
...
read_excel("data/data/wine.xlsx")
```
...
Ask AI to pipe `read_excel("data/wine.xlsx")` into a bar plot with `aes` only mapped to `x = Type` using `geom_bar()`. Add this code as a pipe in continuation of the current code in the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `read_excel` and run `Cmd/Ctrl + Enter`. 
...
Ask AI to start a pipe from `xw` to make a plot using `geom_point()` and the columns `Phenols` and `Flavanoids`. Set `color` argument to `Color` column. Add this code to a new code chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Enter`. 
...
Ask AI to  continue the pipe into the function `geom_jitter()` . Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Enter`. 
...
Ask the same AI to set the `method` argument to `loess` for `geom_smooth()`. Add this code as a replacement of `method = "loess"` to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Enter`. 
...
Ask AI to add a `theme()` argument. Set this to `classic` Add this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Enter`. 
...
Ask AI to use `labs()` and edit or add a proper title, subtitle, and caption. If axis labels are appropriate, add them, but if unnecessary, don't bother. Don't assign the code for the plot to any variable. AAdd this code as a continuation of your pipe to the same chunk in your QMD. Place your cursor on the first line of the code in your current code chunk at the start of `xw` and run `Cmd/Ctrl + Enter`. 
